...
const ContactsList: React.FC<ContactListProps> = ({
    ...
    const contacts: Contact[] = useSelector<State, Contact[]>(
        (state) => state.currentUserContacts
    );

    const filteredContacts = contacts.filter(
        (c) => c.name.indexOf(filter) !== -1 || c.phone.indexOf(filter) !== -1
    );
            ...
            {filteredContacts.length > 0 &&
                filteredContacts.map((c: Contact) => (
                    <ContactItem
                        key={c.contactId}
                        contactInfo={c}
                        onContactEditHandler={onEditContactHandler}
                    />
                ))}
        </div>
    );
